...
interface JobSeekerAuthRequiredProps {
    children: ReactNode;
    redirectTo?: string;
}

/**
 * کامپوننت محافظ برای صفحات مخصوص کارجوها
 * این کامپوننت علاوه بر بررسی احراز هویت، نوع کاربر را نیز بررسی می‌کند
 */
export default function JobSeekerAuthRequired({ children, redirectTo = '/login' }: JobSeekerAuthRequiredProps) {
    ...
    useEffect(() => {
        ...
        if (!isCheckingAuth) {
            ...
            // اگر کاربر احراز هویت شده اما نوع کاربری آن کارجو نیست
            if (!loading && isAuthenticated && user && user.user_type !== 'JS' ) {
                console.log('دسترسی رد شد: کاربر با نوع', user.user_type, 'مجاز به ورود به پنل کارجو نیست.');
                router.push('/');
            }
        }
    ...
    // اگر کاربر احراز هویت شده و نوع کاربری آن کارجو است
    if (!loading && isAuthenticated && user && user.user_type === 'JS' ) {
        return <>{children}</>;
    }
