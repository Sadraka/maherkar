...
interface ResumeAdDetailsProps {
...
const isSpecialSubscription = (resumeAd: ResumeAdvertisement): boolean => {
  ...
  // اگر subscription_detail نبود، از advertisement_detail.subscription بررسی کن
  if (resumeAd.advertisement_detail?.subscription?.subscription_status) {
    if (resumeAd.advertisement_detail.subscription.subscription_status === 'special') {
      return true;
    }
  }
  
  // بررسی بر اساس نام طرح در advertisement_detail
  if (resumeAd.advertisement_detail?.subscription?.plan?.name) {
    const planName = resumeAd.advertisement_detail.subscription.plan.name.toLowerCase();
    if (planName.includes('نردبان') || planName.includes('ویژه') || planName.includes('vip') || planName.includes('ladder')) {
      return true;
    }
  }
...
const ResumeAdDetailsSkeleton: React.FC<{ isMobile: boolean }> = ({ isMobile }) => (
...
export default function ResumeAdDetails({ resumeAd, onClose, onApprove, onReject, onDelete }: ResumeAdDetailsProps) {
  ...
  const fetchSubscriptionData = async () => {
    if (!resumeAd?.advertisement_detail?.subscription && !resumeAd?.subscription_detail) {
    ...
    try {
      ...
      const subscriptionId = resumeAd.advertisement_detail?.subscription?.id || resumeAd.subscription_detail?.id;
      if (subscriptionId) {
      ...
      try {
        ...
        const relatedOrder = allOrders.find((order: any) => {
          ...
          const searchIds = [
            resumeAd.advertisement_detail?.id,
            resumeAd.id,
        ...
        if (relatedOrder) {
          ...
          setSubscriptionData(resumeAd.subscription_detail || resumeAd.advertisement_detail?.subscription);
        }
      } catch (ordersError) {
  ...
  // دریافت اطلاعات اشتراک هنگام تغییر resumeAd
  useEffect(() => {
    fetchSubscriptionData();
  }, [resumeAd?.advertisement_detail?.subscription, resumeAd?.subscription_detail]);
  ...
  useEffect(() => {
    const fetchPayments = async () => {
      ...
      try {
        ...
        if (foundOrders.length === 0 && resumeAd.subscription_orders && resumeAd.subscription_orders.length > 0) {
          
          // تلاش برای دریافت جزئیات هر سفارش
          const orderPromises = resumeAd.subscription_orders.map(async (order: any) => {
            try {
        ...
        if (resumeAd.subscription_orders && resumeAd.subscription_orders.length > 0) {
          const fallbackPayments = resumeAd.subscription_orders.map((order: any) => ({
            id: order.id,
            payment_status: order.payment_status,
            total_price: null,
            durations: null,
    ...
    fetchPayments();
  }, [resumeAd?.advertisement_detail?.id, resumeAd?.id, resumeAd?.subscription_orders]);

  if (!resumeAd) return null;
  
  if (loading) {
    return <ResumeAdDetailsSkeleton isMobile={isMobile} />;
  }
  
  // استفاده از subscriptionData state به جای resumeAd.subscription_detail
  const currentSubscription = subscriptionData || resumeAd.subscription_detail || resumeAd.advertisement_detail?.subscription;
  ...
  return (
    <Box sx={{ p: 2, position: 'relative' }}>
      ...
      {/* کارت اصلی جزئیات آگهی رزومه - مشابه پنل کارفرما با استایل ادمین */}
      <Card
        elevation={0} 
        sx={{ 
        ...
        <Box sx={{ p: { xs: 2.5, sm: 3 } }}>
          <Stack direction="row" alignItems="flex-start" justifyContent="space-between" spacing={2}>
            <Box sx={{ flex: 1, minWidth: 0 }}>
              ...
              >
                {/* اطلاعات کارجو با آواتار بزرگ و مشخص */}
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                  ...
                  {resumeAd.location_detail?.name}
                  {resumeAd.location_detail?.province && `, ${typeof resumeAd.location_detail.province === 'object' && resumeAd.location_detail.province && 'name' in (resumeAd.location_detail.province as any)
                    ? (resumeAd.location_detail.province as any).name
                    : resumeAd.location_detail.province}`}
                </Box>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                  <EventIcon sx={{ fontSize: 16 }} />
                  {resumeAd.created_at ? formatDate(resumeAd.created_at) : 'نامشخص'}
                </Box>
              </Stack>
            </Box>
            ...
            <Stack direction="row" spacing={1}>
              {isSpecialSubscription(resumeAd) && (
                <Chip
                  label="ویژه"
                  size="small"
                  sx={{
              ...
              <Chip
                label={getStatusText(resumeAd.status).text}
                size="small"
                sx={{
                  bgcolor: getStatusText(resumeAd.status).color,
                  color: 'white',
                  fontWeight: 600,
                  fontSize: '0.75rem',
                  px: 1.5,
        ...
        <Box sx={{ p: { xs: 2.5, sm: 3 } }}>
          ...
          <Box sx={{ mb: 4 }}>
            ...
            <Box sx={{ 
              ...
              {/* اطلاعات کارجو */}
              {resumeAd.job_seeker_detail?.username && (
                <Box sx={{ display: 'flex', alignItems: 'center' }}>
                  <Box
                    sx={{
                      backgroundColor: `${ADMIN_THEME.primary}08`,
              ...
              onClick={() => {
                if (typeof window !== 'undefined') {
                  ...
                  if (payments && payments.length > 0 && payments[0].id) {
                    ...
                    window.location.hash = `#payments?search=${encodeURIComponent(resumeAd.job_seeker_detail.full_name)}`;
                  } else if (resumeAd.id) {
                    window.location.hash = `#payments?search=${resumeAd.id}`;
                  } else {
        ...
        <CardContent sx={{ p: 2 }}>
          <Stack spacing={2} alignItems="center">
            ...
            <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.5} justifyContent="center" sx={{ width: '100%' }}>
              {onApprove && (resumeAd.status === 'R' || resumeAd.status === 'P') && (
                <Button
                  variant="contained"
                  size="small"
              ...
              {onReject && (resumeAd.status === 'A' || resumeAd.status === 'P') && (
                <Button
                  variant="contained"
                  size="small"
